...
**Layer move — Phase 1 landed (DDD 2.6):** `Code/Domain/` now physically holds the Godot-free slices: `Common/` (GridPos, Direction), `Combat/` (Damage, HealthController, ICombatant), `Actors/` (IActor, EnemyAI, ArcherAI), `Items/` (Inventory), `Flow/` (TurnManager, TurnState, ExperienceSystem, GameLog), `Grid/FOV/` (FovMap, Raycaster, IFovAlgorithm, VisibilityState). The guard is active. **Not yet moved (graduate when their last Godot bit goes):** `DungeonGrid` (+CellType, generators — partial class, `Vector2I Size`), `GridMover` (`Vector2 WorldPosition`), `EntityManager` (`Node2D` map), `ItemManager`/`IItem` (`Vector2I`). **Phase 2 (pending):** move the Godot view nodes — controllers, TileMaps, UI, Spawner, Main, InputMapper, StairsController/ItemController, `GridConversions` — into `Code/View/` (updates 11 `.tscn` `res://` script paths). `Code/View/` today holds only `GridConversions`. (`Flow/` = the realized name for the turn/progression/log slice; FOV sits under `Domain/Grid/` since the `Dungeon` aggregate hasn't graduated yet.)
