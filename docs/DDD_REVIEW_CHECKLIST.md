...
| 2 | A static (or stateless) service/`*System` that takes an entity and **mutates that entity's internals** | **blocker** | Move the verb onto the entity that owns the state (`attacker.Attack(defender)`). Pure compute-only queries are exempt — they must not write domain state. Note: line-of-sight and pathfinding now live **on `DungeonGrid`** (queries over its own cells; A* runs in a hidden `PathSearch` so its scratch state never persists on the grid), not as free-standing services. Pluggable algorithms stay separate injected interfaces (`IFovAlgorithm`). |
